...
$user_full_name = $content_data['user_full_name'] ?? null;
$user_id = $data['id'] ?? null;
$group_id = $content_data['user_type_id'] ?? null;
?>
    ...
    <div class="col-12">
        <div class="white_card card_height_100 mb_30">
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        ...
                        <div>User Name:-<?=$user_full_name; ?></div>
                    </div>
            ...
            <div class="white_card_body">
                <form method="post" action="<?= site_url('user_menu_access/save') ?>">
...
// ───────────────────────────────────────────────
// Main "All" → affects main row + all submenus completely
// ───────────────────────────────────────────────
...
// ───────────────────────────────────────────────
// When changing individual permission on MAIN MENU
// → sync SAME permission to all submenus
// ───────────────────────────────────────────────
$('.main-menu-checkbox').on('change', function() {
    const row = $(this).data('row');
    const isChecked = this.checked;

    // Extract permission ID from name (e.g. name="3_15" → permission 3)
    ...
    // 2. Cascade SAME permission to ALL submenus
    $(`.sub-menu-checkbox[name^="${permId}_"][data-row="${row}"]`)
        .prop('checked', isChecked);

    // 3. After cascade → update every submenu's "All" checkbox
...
// ───────────────────────────────────────────────
// When changing individual permission on SUBMENU
// ───────────────────────────────────────────────
$('.sub-menu-checkbox').on('change', function() {
    ...
    // Optional: update main "All" only if you want it to reflect full completion
    // Most UIs keep main "All" independent — so usually comment next block
    /*
    const $mainPerms = $(`.main-menu-checkbox[data-row="${row}"]`);
    updateAllState($(`#all_${row}`), $mainPerms);
    */
...
// ───────────────────────────────────────────────
// Initial sync (very important on page load / edit mode)
// ───────────────────────────────────────────────
